...
app = FastAPI(title="ZeroDollar Email Verifier", version="0.2.0")

# ---- API Key guard (env-based) ----
_ALLOWED = {k.strip() for k in os.getenv("API_KEYS", "").split(",") if k.strip()}

def require_api_key(x_api_key: str | None = Header(default=None), key: str | None = Query(default=None)):
    supplied = x_api_key or key
    if not _ALLOWED or not supplied or supplied not in _ALLOWED:
        # 401 with a WWW-Authenticate hint is nice
        raise HTTPException(status_code=401, detail="Missing/invalid API key", headers={"WWW-Authenticate": "ApiKey"})
    return True
# -----------------------------------
...
async def verify_batch(payload: dict):
    emails = [e for e in payload.get("emails", []) if isinstance(e, str)]
    return JSONResponse({"results": [await verify_email_address(e) for e in emails]})
